...
打开 **{{SITE}}/**,页面底部「模块订阅地址」一节列出了全部五种格式,长按复制对应你客户端的那一条:

| 客户端 | 模块地址 |
| --- | --- |
| Surge / Egern | `{{SITE}}/modules/wloc.sgmodule` |
| Quantumult X | `{{SITE}}/modules/wloc.conf` |
| Loon | `{{SITE}}/modules/wloc.lpx` |
| Stash | `{{SITE}}/modules/wloc.stoverride` |
| Shadowrocket | `{{SITE}}/modules/wloc.module` |
...
```text
{{SITE}}/api/parse?format=json&u=<地图分享链接>
```
