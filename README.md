...
用 Safari 打开部署后的选点页 → 页面底部「免客户端模式」卡片 → **填上你的 Wi-Fi 名称**(每行一个,带密码写成 `SSID | 密码`)→ 点 **生成描述文件并下载** → 按提示安装。

一份描述文件同时做两件事:**装根证书** + **把这些 Wi-Fi 的代理指向电脑**(代理用 PAC,见下)。然后**必须**再走一步:

**设置 → 通用 → 关于本机 → 证书信任设置** → 对 `selfhost-wloc Root CA` 打开**完全信任**。

> [!IMPORTANT]
> **装证书 ≠ 完全信任证书。** 漏掉这一步的典型现象是「网络连接已中断」「保存成功但地图不变」。
>
> 非监管设备上 iOS 只允许**按 Wi-Fi** 配代理(SSID 是必填的),所以描述文件必须要知道 Wi-Fi 名。**换新 Wi-Fi 就把新名字加进去重新生成装一次**(证书重复装没有副作用)。不想用描述文件配代理的话,也可以只装证书,再到 设置 → 无线局域网 → ⓘ → 配置代理 → **自动** 里手填 PAC 网址。
>
> **代理用 PAC 而不是"手动代理 + IP"。** 手动代理是全局的:引擎没开时,该 Wi-Fi 上**所有**流量都会被送去一个不通的地址,会断网。PAC 只把那三个定位域名送进电脑,其余全部直连,而且**带回退**——引擎没开时定位请求自动直连,其它上网完全不受影响。
>
> **电脑地址不在手机上。** 它由站点的 PAC 提供:填进 `project.config.json` 的 `engineHost`(主机名或固定 IP),推送后自动生效——**以后电脑换地址只改这一处,手机端零改动**。想临时换地址也可以在手机的「自动代理」里直接写成 `<站点>/wloc.pac?h=新地址&p=端口`。
>
> 站点分发的证书来自仓库 `public/ca.cer`(只有证书、没有私钥;私钥始终在引擎所在机器),由 `engine/tools/make-certs.sh` 自动同步——**重新生成证书后必须重跑 `npm run configure` 并提交**,否则站点发出的是旧证书。页面上的指纹可用来核对。
