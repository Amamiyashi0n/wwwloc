...
2. 设置 → 无线局域网 → 当前 Wi-Fi 的 ⓘ → 配置代理 → **手动** → 服务器填**电脑的局域网 IP**、端口 `8888`

> [!IMPORTANT]
> **代理地址不能填 `127.0.0.1`。** 这个字段是「手机要连到哪里」,`127.0.0.1` 指手机自己,而手机上没有任何东西在监听(本方案不装 App),填了会直接断网。也不能填引擎日志里的 `0.0.0.0`(那是监听所有网卡的意思,不是可连接地址)。**建议把电脑 IP 固定下来**(DHCP 保留或静态 IP),否则 IP 一变就要重新改。
>
...
想让描述文件把代理一并配好(免去第 2 步)?展开页面上的「进阶:让描述文件自动配置代理」,填电脑 IP 与 Wi-Fi 名即可。
