...
手机与电脑连同一 Wi-Fi,用 Safari 打开部署后的选点页 → 页面底部「免客户端模式」卡片 → 点 **安装根证书**(一个按钮)→ 按提示安装。

然后**必须**再走两步:

1. 设置 → 通用 → 关于本机 → **证书信任设置** → 对 `selfhost-wloc Root CA` 打开**完全信任**
2. 设置 → 无线局域网 → 当前 Wi-Fi 的 ⓘ → 配置代理 → **自动** → 网址填页面上显示的 **PAC 网址**(`<你的站点>/wloc.pac`)

> [!IMPORTANT]
> **不要用手动代理 + IP。** 手动代理是全局的:引擎没开时,该 Wi-Fi 上**所有**流量都会被送去一个不通的地址,会断网。PAC 只把那三个定位域名送进电脑,其余全部直连,而且**带回退**——引擎没开时定位请求自动直连,其它上网完全不受影响。
>
> **PAC 网址里没有 IP。** 电脑地址放在站点这一侧:填进 `project.config.json` 的 `engineHost`(主机名或固定 IP),推送后自动生效。**以后电脑换地址只改这一处,手机端零改动。** 想临时换地址也可以在手机的「自动代理」里直接写成 `<站点>/wloc.pac?h=新地址&p=端口`。
>
> **装证书 ≠ 完全信任证书。** 漏掉第 1 步的典型现象是「网络连接已中断」「保存成功但地图不变」。
>
...
想省掉第 2 步的手动配置?展开页面上的「进阶:把代理写进描述文件」,填上你的 Wi-Fi 名即可(可写多个,每行一个)。
