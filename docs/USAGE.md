...
一份描述文件同时完成两件事:**装根证书** + **把你列出的 Wi-Fi 的代理指向电脑**。

1. 用 **Safari** 打开 **https://wwwloc-page.amamiyashion.workers.dev/**
2. 在「免客户端模式」卡片里填 **Wi-Fi 名称**——每行一个,带密码写成 `SSID | 密码`,例如:
...
3. 点「生成描述文件并下载」→ 系统提示下载完成
4. 设置 → 通用 → **VPN与设备管理** → 安装刚下载的描述文件
5. **根证书已自动载入**(页面显示 SHA-256 指纹),不需要手动传文件
...
> [!NOTE]
> **换新 Wi-Fi 怎么办?** 把新 Wi-Fi 的名字加进那个框,重新生成装一次即可(证书重复装没有副作用)。这不是方案偷懒 —— 非监管设备上 iOS 只允许**按 Wi-Fi** 配代理(`SSID` 是必填字段),没有"所有网络通用"的写法。

> [!NOTE]
> **不想用描述文件配代理?** 也可以只下载根证书,然后到 设置 → 无线局域网 → 当前 Wi-Fi 的 ⓘ → 配置代理 → **自动**,网址填 `https://wwwloc-page.amamiyashion.workers.dev/wloc.pac`。这样换 Wi-Fi 时按同样步骤再填一次即可。
