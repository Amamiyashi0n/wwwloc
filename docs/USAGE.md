...
装好后回到「设置 → 无线局域网」,点当前 Wi-Fi 右侧的 ⓘ,确认底部 **HTTP 代理 = 手动**,服务器是电脑 IP、端口 8888。如果没生效,手动重连一次该 Wi-Fi。

> [!IMPORTANT]
> **代理地址必须是电脑的局域网 IP(如 `192.168.1.100`),不能填 `127.0.0.1`。**
>
> 这个字段是「手机要连到哪里去」——`127.0.0.1` 指的是**手机自己**,而手机上并没有任何东西在监听(本方案不装 App),填了会直接断网。同理也不能填引擎日志里的 `0.0.0.0`(那是"监听所有网卡"的意思,不是可连接地址)。
>
> **电脑 IP 建议固定下来**(路由器里做 DHCP 保留,或在系统里设静态 IP):IP 一变,手机上的代理就指不到了,需要重新改。
...
3. **等约 10 秒**
4. **关闭飞行模式**
5. **等网络恢复**:确认 Wi-Fi 已连上、ⓘ 里 HTTP 代理仍是电脑 IP、引擎仍在运行。**不要这时候急着开定位**
6. **最后重新开启定位服务**
